'''hw4p2.py
   ...
   This is skeleton code for HW4 Problem 2.  PLEASE EDIT (SEARCH FOR FIXME)!
...
#
#  Utilities - Use for columns (d) and (e).
#
...
def newton_raphson(xgoal):
    ...
    for i in range(N):
        xdistance.append(np.linalg.norm(xgoal - fkin(q)))
        prev_q = q
        q = q + (np.linalg.inv(Jac(q)) @ (xgoal - fkin(q)))
        qstepsize.append(np.linalg.norm(q - prev_q))
        if(np.linalg.norm(xgoal-fkin(q)) < 1e-12):
            print(f"converges: {i}")
    print(f"x goal: {xgoal}")
    print(f"final q: {np.round(q, decimals=4)}")
    print(f"wraps: {wraps(q)}")
    print(f"elbow up?: {elbow_up(q)}")
    print(f"front side?: {front_side(q)}")
    print("\n")
