...
class TrajectoryNode(Node):
    ...
    def update(self):
        ...
        if self.t <= 2.0:
            # Part A (t<=2):
            (alpha, alphadot) = goto(self.t, 2.0, 0.0, np.radians(-90))
            (beta,  betadot)  = (0.0, 0.0)

            # Set up the desired rotation and angular velocity.
            Rd = Roty(alpha)
            wd = np.array([0, alphadot, 0])

        else:
            # Part B (t>2):
            (alpha, alphadot) = (np.radians(-90), 0.0)
            (beta,  betadot)  = (self.t - 3 + np.exp(2 - self.t), 1 - np.exp(2 - self.t))

            n = np.array([0, 2/np.sqrt(5), -1/np.sqrt(5)])

            # Set up the desired rotation and angular velocity.
            Rd = Roty(alpha) @ Rotn(n, beta)
            wd = betadot * n
        ...
        qcdot = np.linalg.inv(Jac(qclast)) @ (wd + self.lam * elast)
