...
class TrajectoryNode(Node):
    ...
    def __init__(self, name, future):
        ...
        self.qc = self.qA
        self.qcdot = np.array([0.0, 0.0, 0.0])

        self.Rd = np.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
    ...
    def update(self):
        ...
        # First leg
        if(t < self.T):
            # Compute a spline between xA and xF
            (xd, vd) = goto(t , self.T, self.xA, self.xF)

            # Calculate qcdot and qc using inverse kinematics
            qcdot = np.linalg.inv(Jac(self.qc)) @ (vd + self.lamb * self.pos_err)
            qc = self.qc + self.dt * qcdot
            self.qcdot = qcdot
            self.qc = qc

            # Set qd and qddot from qc and qcdot
            pd = fkin(qc)
            vd = Jac(qc) @ qcdot

            # Calculate new position error
            xc = fkin(qc)
            self.pos_err = xd - xc
        # Second leg
        else:
            # Use the inverse kinematics from the first leg to get qF
            if(self.qF is None):
                self.qF = self.qc

            # Compute a spline between qA and qF
            (qd, qddot) = goto(t - self.T , self.T, self.qF, self.qA)

            # Compute pd and vd
            pd = fkin(qd)
            vd = Jac(qd) @ qddot

            # Set qc and qcdot directly from spline
            qc = qd
            qcdot = qddot
            self.qc = qc
            self.qcdot = qcdot
