...
class TrajectoryNode(Node):
    ...
    def update(self):
        ...
        mid_qddot = np.array([-1.178, 0.0, 0.0])

        # Compute the joint values
        if   (t < 2.0): 
            (qd, qddot) = spline(t     , 2.0, self.qA, self.qB, np.array([0.0, 0.0, 0.0]), mid_qddot)
        elif (t < 4.0): 
            (qd, qddot) = spline(t-2.0 , 2.0, self.qB, self.qC, mid_qddot, np.array([0.0, 0.0, 0.0]))
        else:           
            (qd, qddot) = goto(t-4.0 , 2.0, self.qC, self.qA)
