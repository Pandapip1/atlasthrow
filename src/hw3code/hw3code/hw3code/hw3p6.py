...
hw3p6.py
   ...
   This is a skeleton for HW3 Problem 6. 
...
class TrajectoryNode(Node):
    ...
    def update(self):
        ...
        theta_1  = np.sin(2 * self.t)
        omega_1  = 2 * np.cos(2 * self.t)

        theta_2  = np.cos(2 * self.t)
        omega_2  = 2 * -np.sin(2 * self.t)

        theta_3  = np.sin(self.t)
        omega_3  = np.cos(self.t)

        theta_4  = np.cos(self.t)
        omega_4  = -np.sin(self.t)

        # We build up numpy arrays, consistent with future computations.
        q    = np.array([theta_1, theta_2, theta_3, theta_4])
        qdot = np.array([omega_1, omega_2, omega_3, omega_4])
