...
class TrajectoryNode(Node):
    ...
    def update(self):
        ...
        theta_pan  = np.pi / 3 * np.sin(2 * self.t)
        omega_pan  = 2 * np.pi / 3 * np.cos(2 * self.t)

        theta_tilt =  np.pi / 3 * np.sin(self.t) - np.pi / 9 * np.cos(6 * self.t)
        omega_tilt =  np.pi / 3 * np.cos(self.t) + 2 * np.pi / 3 * np.sin(6 * self.t)
