...
class TrajectoryNode(Node):
    ...
    def update(self):
      ...
      qdsecdot = np.array([-1/4*np.pi, -1/4*np.pi, np.pi/2, -np.pi/2, 0, 0, 0]).T
      ...
      qsecrdot = qdsecdot + self.lamsec * eseclast

      # Compute the inverse kinematics
      qcdot = np.linalg.pinv(J) @ np.concatenate((vr, wr))
      qcdot = qcdot + (np.identity(7) - np.linalg.pinv(J) @ J) @ qsecrdot
