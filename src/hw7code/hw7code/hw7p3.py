...
class TrajectoryNode(Node):
    ...
    def __init__(self, name, future):
      ...
      alpha = np.radians(46.5675)
      self.q0 = np.radians(np.array([0, alpha, 0, -2 * alpha, 0, 0, alpha]))
    ...
    def update(self):
      ...
      pd = np.array([0, 0.95 - 0.25 * np.cos(self.t), 0.60 + 0.25 * np.sin(self.t)])
      vd = np.array([0, 0.25 * np.sin(self.t), 0.25 * np.cos(self.t)])
      ...
      J = np.vstack((Jv, Jw))
      Jmod = np.vstack((J, np.array([0, 0, 0, 1, 0, 0, 0])))

      # Calculate vr and wr
      vr = vd + self.lam * eposlast
      wr = wd + self.lam * erotlast
      secr = secd + self.lam2 * eseclast
      xr = np.concatenate((vr, wr))
      xr = np.concatenate((xr, np.array([secr])))

      # Compute the inverse kinematics
      W = np.diag([1, 1, 1, 1, 1, 1, 0.2])
      qcdot = np.linalg.pinv(Jmod.T @ W @ Jmod) @ Jmod.T @ W @ xr
      ...
      Jbar = np.diag(np.array([1/self.L, 1/self.L, 1/self.L, 1, 1, 1])) @ J
