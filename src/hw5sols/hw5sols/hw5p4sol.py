...
class TrajectoryNode(Node):
    ...
    def update(self):
        ...
        if self.t <= 2.0:
            # Part A (t<=2):
            (alpha, alphadot) = goto(self.t, 2.0, 0.0, -pi/2)
            (beta,  betadot)  = (0.0, 0.0)
        else:
            # Part B (t>2):
            (alpha, alphadot) = (-pi/2, 0.0)
            (beta,  betadot)  = (self.t-3+exp(2-self.t), 1-exp(2-self.t))
        ...
        if False:
            ...
            nA = nxyz(0.0, 1/sqrt(2), -1/sqrt(2))
            Rd = Roty(alpha) @ Rotn(nA, beta)

            wd = ny() * alphadot + Roty(alpha) @ nA * betadot
        else:
            # Use the diagonal axis expressed in the O frame.
            nO = nxyz(1/sqrt(2), 1/sqrt(2), 0.0)
            Rd = Rotn(nO, beta) @ Roty(alpha)

            wd = nO * betadot + Rotn(nO, beta) @ ny() * alphadot

        # Compute the reference velocity.
        xrdot = wd + self.lam * elast
