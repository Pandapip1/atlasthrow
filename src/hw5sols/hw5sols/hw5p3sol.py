...
hw5p3sol.py
   ...
   This is the solution code for HW5 Problem 3.
...
class TrajectoryNode(Node):
    ...
    def update(self):
        ...
        if (t < self.T):
            ...
            # Compute the task spline.
            (pd, vd) = goto(t, self.T, self.xA, self.xF)

            # Compute the reference velocity.
            xrdot = vd + self.lam * elast

            # Compute the inverse kinematics
            J     = Jac(qclast)
            qcdot = np.linalg.inv(J) @ xrdot

            # Integrate the joint command position.
            qc = qclast + self.dt * qcdot

            # Compute the equivalent task command.
            pc = fkin(qc)
            vc = Jac(qc) * qcdot            # Not used
        ...
        # Leg 2 - joint spline
            ...
            # Check whether we know where qF is.
            if self.qF is None:
                self.qF = self.qc

            # Compute the joint spline.
            ...
            # Simply match the command to the desired joint values.
            (qc, qcdot) = (qd, qddot)
